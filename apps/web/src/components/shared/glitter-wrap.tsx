...
export function GlitterWrap(props: GlitterWrapProps) {
    ...
    const colorCacheRef = useRef({
        ...
        parsed2: [255, 255, 255, 1] as [number, number, number, number],
    ...
    const getCachedColors = () => {
        ...
        if (pr.color1 !== c.color1) {
            c.color1 = pr.color1
            c.parsed1 = parseColor(pr.color1)
        }
        if (pr.color2 !== c.color2) {
            c.color2 = pr.color2
            c.parsed2 = parseColor(pr.color2)
        }
        if (pr.color3 !== c.color3) {
            c.color3 = pr.color3
            c.parsed3 = parseColor(pr.color3)
        }
        return c
    }
    ...
    useEffect(() => {
        ...
        const cfg = () => {
            ...
            return {
                reverse: pr.reverse,
                density: pr.density,
                stepZ: pr.speed * 0.0008,
                focalDepth: pr.focalDepth / 100,
                starScale: pr.starSize * 0.15,
                turbulence: pr.turbulence * 0.2,
                glitter: pr.glitterIntensity * 0.1,
                brightness: Math.min(1, pr.brightness / 100),
                trail: pr.trailAmount / 100,
            }
        }
